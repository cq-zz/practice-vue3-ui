...
const getTplFilePath = (meta) => ({
  ...
  readme: {
    ...
    to: `../../packages/${meta.componentName}/docs/README.md`
  },
  demo: {
    from: './.template/docs/demo.vue.tpl',
    to: `../../packages/${meta.componentName}/docs/demo.vue`
  },
  // src 目录
  vue: {
    from: './.template/src/index.vue.tpl',
    to: `../../packages/${meta.componentName}/src/index.vue`
  },
  // 根目录
  install: {
    from: './.template/index.ts.tpl',
    to: `../../packages/${meta.componentName}/index.ts`
  },
})
...
const routerTplReplacer = (listFileContent) => {
  ...
  const routerMeta = {
    routes: listFileContent.map(comp => {
      return `{
    title: '${comp.componentZhName}',
    name: '${comp.componentName}',
    path: '/components/${comp.componentName}',
    component: () => import('packages/${comp.componentName}/docs/README.md'),
  }`
    })
  }
...
const installTsTplReplacer = (listFileContent) => {
  ...
  const installMeta = {
    importPlugins: listFileContent.map(({ componentName }) => `import { ${componentName}Plugin } from './${componentName}';`).join('\n'),
    installPlugins: listFileContent.map(({ componentName }) => `${componentName}Plugin.install?.(app);`).join('\n    '),
    exportPlugins: listFileContent.map(({ componentName }) => `export * from './${componentName}'`).join('\n'),
  }
...
module.exports = (meta) => {
  ...
  console.log(`组件新建完毕，请前往 packages/${meta.componentName} 目录进行开发`);
}
